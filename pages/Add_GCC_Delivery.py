...
# Constants
CONFIG_FILE = 'deliveries_config3.json'
# S3_BUCKET = 'your-config-bucket'  # Replace with your actual bucket name
...
FUNCTION_MAP = {
    ...
    "removing_reits": {
        "name": "Remove all REITs",
        "function": "removing_reits",
        "args": {},
        "description": "Remove all REITs"
}
}
...
def render_output_config(output_index: int) -> Dict:
    ...
    with st.container():
        ...
        config["filename_pattern"] = st.text_input(
            ...
            help="Available placeholders: {client}, {year}, {month}, {day}, {timestamp}"
        )
...
#                 config["deliveries"].append(new_delivery)
#
#                 with open('deliveries_config3.json', 'w') as f:
#                     json.dump(config, f, indent=2)
#
#                 st.success("Delivery configuration saved successfully!")
#                 st.balloons()
